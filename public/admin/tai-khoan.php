...
{
    $delete = $_GET['delete'];
    $create = $NTA->delete("taikhoan","`id` = $delete");
    if ($create)
    {
      echo '<script type="text/javascript">swal.fire("Thành Công","Xóa thành công","success");setTimeout(function(){ location.href = "tai-khoan.php" },500);</script>'; 
    }
    else
    {
      echo '<script type="text/javascript">swal.fire("Thất Bại","Lỗi máy chủ","error");setTimeout(function(){ location.href = "tai-khoan.php" },1000);</script>'; 
    }
}
?>
<?php
if( isset($_POST['btnAddClone']) && isset($_SESSION['admin']) )
{
  ...
  foreach($clone as $row)
  {
    ...
    $check_chung_clone = $NTA->num_rows("SELECT * FROM `taikhoan` WHERE `id_fb` = '".$get_string_clone[0]."' ");
    if($check_chung_clone  > 0)
    {
      $NTA->update("taikhoan",[
        'code'=>$loai,
        'status'=>'live',
  ...
  echo '<script type="text/javascript">swal.fire("Thành Công","Thêm '.format_cash($countadd).' clone | Cập nhật '.format_cash($countupdate).' clone","success");</script>'; 
}
...
if( isset($_POST['btnDeleteClone']) && isset($_SESSION['admin']) )
{
  ...
  foreach($clone as $row)
  {
    ...
    $NTA->delete("taikhoan","`id_fb` = $data");
    $count++;
  }
  echo '<script type="text/javascript">swal.fire("Thành Công","Đã xóa '.format_cash($count).' clone thành công !","success");</script>'; 
}
?>
...
<?php
$i = 0;
$result = $NTA->get_list("SELECT * FROM `taikhoan` WHERE `username` IS NULL ORDER BY id desc");
if ($result){
  foreach ($result as $row){
  
  ?>
                    <tr>
                      <td class="text-center"><?=$row['id'];?> <?php $i++;?></td>
                      <td class="text-center"><?=$row['id_fb'];?></td>
                      <td class="text-center"><?= $NTA->get_row("SELECT * FROM `category` WHERE `code` = '".$row['code']."'")['title'];?></td>
          ...
          <div class="form-group">
            ...
            <div class="form-group">
              ...
              <select class="custom-select"  name="loai" >
                  <?php
                  $result = $NTA->get_list("SELECT * FROM `category`");
                  if ($result){
