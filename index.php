<?php require_once("data/head.php") ?>
<?php require_once("data/nav.php") ?>
<style>
    .cash {
        border-radius: 20px;
        border: 2px dotted red;
    }
</style>
<div class="content d-flex flex-column flex-column-fluid" id="kt_content" style="margin-top: 2%;">
    <div class="d-flex flex-column-fluid">
        <div class="container">
            <div class="row">
                <?php if (empty($_SESSION['username'])) { ?>
                    <div class="col-xl-12">
                        <div class="row">
                            <div class="col-lg-12 col-xxl-12">
                                <div class="card card-custom bgi-no-repeat gutter-b" style="height: 175px; background-color: rgb(247, 170, 71); background-position: calc(95% + 0.5rem) 37%; background-size: 50%; background-image: url(/img/z2.png);">
                                    <div class="card-body d-flex align-items-center">
                                        <div>
                                            <h3 class="text-white font-weight-bolder line-height-lg mb-5"> localhost | CLONE | XU TƯƠNG TÁC <br>XIN CHÀO QUÝ KHÁCH </h3>
                                            <a href="/login/" class="btn btn-primary font-weight-bold mr-1">Đăng Nhập</a><a href="/register" class="btn btn-danger font-weight-bold">Đăng Ký</a>
                                        </div>
                                    </div>
                                </div>
                                <div class="row"></div>
                            </div>
                        </div>
                    </div>
                <?php } else { ?>
                    <div class="col-xl-12">
                        <div class="row">
                            <div class="col-md-4">
                                <div class="card mb-4 cash">
                                    <div class="card-body d-flex flex-column text-center gap-2">
                                        <div class="display-7">
                                            <i class="fa fa-money text-success"></i>
                                        </div>
                                        <div class="lead">Số dư <b class="text-danger"><?= format_cash($user['money']) ?></b> <sup>đ</sup></div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4 col-12">
                                <div class="card mb-4 cash">
                                    <div class="card-body d-flex flex-column text-center gap-2">
                                        <div class="display-7">
                                            <i class="fa fa-money text-primary"></i>
                                        </div>
                                        <div class="lead">Đã nạp <b class="text-danger"><?= format_cash($user['total_nap']) ?></b> <sup>đ</sup></div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4 col-12">
                                <div class="card mb-4 cash">
                                    <div class="card-body d-flex flex-column text-center gap-2">
                                        <div class="display-7">
                                            <i class="fa fa-money text-info"></i>
                                        </div>
                                        <div class="lead">Nạp tháng <b class="text-danger">0</b> <sup>đ</sup></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="col-xl-12" ">
                        <div class=" row">
                        <div class="col-xl-12">
                            <!--begin::Mixed Widget 10-->
                            <div class="card card-custom gutter-b example example-compact bg-light-danger" style="background-color: #ffc107;border: 2px dotted red;">
                                <div class="card-header" style="margin-top:2%;">
                                    <h1 style="text-align: center;color: #ff00ff;"><?= $site_tenweb ?> THÔNG BÁO</h1>
                                </div>
                                <div class="col-lg-12" style="margin-top:5px;">
                                    <div class="card-body">
                                        <h4>
                                            <?= $site['thongbao'] ?>
                                        </h4>


                                    </div>
                                </div>
                            </div>
                            <!--end::Mixed Widget 10-->
                        </div>
                    </div>
            </div>
        <?php } ?>



        <div class="d-lg-flex flex-row-fluid" style="margin-top: 2%;">
            <div class="content-wrapper flex-row-fluid">
                <?php
                $chuyenmuc = $NTA->get_list("SELECT * FROM `chuyenmuc_clone` WHERE `display` = 'show'");
                if ($chuyenmuc) {
                    foreach ($chuyenmuc as $data) {
                        $loai = $data['code'];
                        $result = $NTA->query("SELECT * FROM `category` WHERE `display` = 'show' AND pin = '$loai' ");

                ?>

                        <div class="card mb-4 " style="border: 2px dotted red;margin-top:2%">
                            <div class="card-body">
                                <div class="ribbon-title ribbon-primary" style="margin-top: 1%;margin-left: 1%">
                                    <h5><?= $data['name'] ?></h3>
                                </div>
                                <br>

                                <div class="mt-4">
                                    <div class="table-responsive">
                                        <table class="table table-striped table-bordered table-hover">
                                            <thead>
                                                <tr role="row" class="bg-primary">
                                                    <th class="text-center text-white">Sản phẩm</th>
                                                    <th class="text-center text-white">Hiện còn</th>
                                                    <th class="text-center text-white">Đã bán</th>
                                                    <th class="text-center text-white">Giá</th>
                                                    <th class="text-center text-white">Hành động</th>
                                                </tr>
                                            </thead>
                                            <?php
                                            if ($result->num_rows > 0) {
                                            ?>
                                                <tbody role="alert" aria-live="polite" aria-relevant="all" class="">
                                                    <?php while ($row = mysqli_fetch_array($result)) { ?>

                                                        <?php $clone_conlai = $NTA->allsp("taikhoan", "`code` = '" . $row['code'] . "' AND `username` IS NULL AND `status` = 'live' "); ?>
                                                        <?php $clone_daban = $NTA->allsp("taikhoan", "`code` = '" . $row['code'] . "' AND `username` IS NOT NULL "); ?>


                                                        <tr>
                                                            <td class="py-8 text-center">
                                                                <div class="d-flex align-items-center text-left">
                                                                    <div>
                                                                        <a href="#" class="text-dark-75 text-hover-primary mb-1 font-size-lg">
                                                                            <b><?= $row['title'] ?></b>
                                                                        </a>
                                                                        <span class="text-muted font-weight-bold d-block"><?= $row['note'] ?></span>
                                                                    </div>
                                                                </div>
                                                            </td>
                                                            <td class="text-center"> <span class="badge bg-success text-white"><?= $clone_conlai ?>
                                                                    tài
                                                                    khoản</span>
                                                            </td>
                                                            <td class="text-center"> <span class="badge bg-info text-white"><?= $clone_daban ?>
                                                                    tài
                                                                    khoản</span>
                                                            </td>
                                                            <td class="text-center"><b class="text-danger"><?= format_cash($row['money']); ?></b>
                                                                <sup>đ</sup>
                                                            </td>
                                                            <td class="text-center">
                                                                <div class="flex align-items-center">
                                                                    <?php if ($clone_conlai < 1) { ?>
                                                                        <button class="btn btn-danger  btn-block" disabled=""><i class="fa fa-shopping-cart"></i> Hết hàng</button>
                                                                    <?php } else { ?>
                                                                        <button class="btn btn-success  btn-block" data-toggle="modal" data-target="#modal_<?= $row['code'] ?>"><i class="fa fa-shopping-cart"></i> Mua Hàng</button>
                                                                    <?php } ?>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                        <?php
                                                        if (empty($_SESSION['username'])) { ?>
                                                            <div class="modal fade" id="modal_<?= $row['code']; ?>" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                                                                <div class="modal-dialog modal-dialog-centered">
                                                                    <div class="modal-content">
                                                                        <div class="modal-header">
                                                                            <h3><img src="https://subgiare.vn/assets/images/svg/notification.svg" alt="" width="25"> Thông Báo</p>
                                                                        </div>
                                                                        <div class="modal-body">

                                                                            <div class="pt-3 text-center">
                                                                                <h3>Bạn chưa đăng nhập, vui lòng đăng nhập để tiếp tục !</h3>
                                                                            </div>

                                                                        </div>
                                                                        <div class="modal-footer">
                                                                            <div class="d-grid gap-2">
                                                                                <button type="button" class="btn btn-danger" data-dismiss="modal">Đóng Thông Báo</button>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        <?php } else { ?>
                                                            <div class="modal fade" id="modal_<?= $row['code']; ?>" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                                                                <div class="modal-dialog modal-dialog-centered">
                                                                    <div class="modal-content">
                                                                        <div class="modal-body">
                                                                            <form method="post" action="" id="formMua<?= $row['code']; ?>">
                                                                                <h3 class="text-center">Xác Nhận Mua Hàng</h3>
                                                                                <br>
                                                                                <div class="alert alert-primary">
                                                                                    <p>Tên Sản Phẩm : <?= $row['title'] ?></p>
                                                                                    <p>Giá 1 sản phẩm : <?= format_cash($row['money']); ?>đ </p>
                                                                                    <p>Hiện Có : <?= $clone_conlai ?> Tài Khoản</p>
                                                                                    <?php if ($row['note']) { ?>
                                                                                        <hr>
                                                                                        <?= $row['note']; ?>
                                                                                    <?php } ?>
                                                                                </div>

                                                                                <input type="text" value="<?= $row['code']; ?>" id="code_<?= $row['code']; ?>" hidden="">
                                                                                <input type="text" value="<?= $row['pin']; ?>" id="loai_<?= $row['code']; ?>" hidden="">
                                                                                <label>Số lượng cần mua: <b id="error" style="color:red"></b></label>
                                                                                <div class="input-group custom">
                                                                                    <input type="number" id="soluong_<?= $row['code']; ?>" name="soluong" class="form-control form-control-lg" placeholder="Nhập số lượng cần mua" required>
                                                                                </div>
                                                                                <br>
                                                                                <button type="button" id="buy<?= $row['code']; ?>" class="btn btn-success" data-dismiss="modal">Thanh
                                                                                    toán</button>
                                                                                <button type="button" id="btnDong<?= $row['code']; ?>" class="btn btn-danger" data-dismiss="modal">Đóng</button>
                                                                            </form>
                                                                        </div>
                                                                        <div class="modal-footer">
                                                                            Tổng tiền cần thanh Toán :<span class="font-weight-bold"><b id="ketqua_<?= $row['code']; ?>">0</b>đ</span>

                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <script type="text/javascript">
                                                                $('#soluong_<?= $row['code']; ?>').keyup(function() {
                                                                    var soluong = $("#soluong_<?= $row['code']; ?>").val();
                                                                    var ketqua1 = <?= $row['money']; ?> * soluong;
                                                                    var ketqua = ketqua1 - ketqua1 * <?= $user['ck']; ?> / 100;
                                                                    $('#ketqua_<?= $row['code']; ?>').html(ketqua.toString().replace(
                                                                        /(.)(?=(\d{3})+$)/g, '$1,'));
                                                                });

                                                                $(document).ready(function() {
                                                                    $("#buy<?= $row['code'] ?>").click(function() {

                                                                        var code = $("#code_<?= $row['code'] ?>").val();
                                                                        var loai = $("#loai_<?= $row['code'] ?>").val();
                                                                        var soluong = $("#soluong_<?= $row['code'] ?>").val();
                                                                        if (soluong == '') {
                                                                            swal("Lỗi", "Vui Lòng Điền Số Lượng muốn Mua", "error")
                                                                        } else if (soluong <= 0) {
                                                                            swal("Lỗi", "Vui Lòng Mua ít Nhất 1 Tài Khoản", "error")
                                                                        } else {
                                                                            $.post("/api/buy", {
                                                                                code: code,
                                                                                loai: loai,
                                                                                soluong: soluong
                                                                            }, function(data) {
                                                                                console.log(data);
                                                                                if (data['status'] == "error") {

                                                                                    swal("Lỗi", data['msg'], "error");
                                                                                } else if (data['status'] == "warning") {
                                                                                    swal("Lỗi", "Vui Lòng Đăng Nhập Để Tiếp Tục", "error");
                                                                                } else if (data['status'] == "success") {
                                                                                    swal("Thành Công", data['msg'], "success");
                                                                                    setTimeout(function() {
                                                                                        location.href = "/order/view/" + data['id'] + "/"
                                                                                    }, 1000);
                                                                                } else {
                                                                                    swal("Lỗi", "Lỗi Không Xác Định, Vui Lòng Thử Lại", "warning");
                                                                                }
                                                                            }, "json");
                                                                        }
                                                                    });

                                                                });
                                                            </script>

                                                    <?php }
                                                    } ?>
                                                </tbody>
                                            <?php } else { ?>
                                                <tbody role="alert" aria-live="polite" aria-relevant="all" class="">
                                                    <tr class="odd">
                                                        <td valign="top" colspan="100%">
                                                            <div class="text-center">
                                                                <img src="data:image/webp;base64," alt="nodataa" width="200" height="150">
                                                                <br><b>Sản Phẩm Đang Tạm Hết, Vui Lòng Quay Lại Sau...</b>
                                                            </div>
                                                        </td>
                                                    </tr>
                                                </tbody>
                                            <?php } ?>
                                        </table>
                                    </div>
                                </div>
                            </div>
                        </div>

                <?php
                    }
                }
                ?>
            </div>
        </div>

        <div class="row">
    <div class="col-md-6">
        <div class="card card-custom card-stretch gutter-b">
            <div class="card-header align-items-center border-0 mt-4">
                <h3 class="card-title align-items-start flex-column">
                    <span class="font-weight-bolder text-dark">GIAO DỊCH GẦN ĐÂY</span>
                                    </h3>
            </div>
                        <div class="card-body pt-4">
                <div class="timeline timeline-6 mt-3">
                    <div class="timeline-item align-items-start">
                        <div class="timeline-label font-weight-bolder text-dark-75 font-size-lg"> 11 ngày </div>
                            <div class="timeline-badge"><i class="fa fa-genderless text-danger icon-xl"></i></div>
                            <div class="timeline-content font-weight-bolder font-size-lg text-dark-75 pl-3"> ******** : Vừa mua 1 tài khoản Tài khoản TDS 1m Không Cấu Hình (Bảo hành 72h) với giá 13.000đ
                        </div>
                    </div>
                </div>
            </div>
                        <div class="card-body pt-4">
                <div class="timeline timeline-6 mt-3">
                    <div class="timeline-item align-items-start">
                        <div class="timeline-label font-weight-bolder text-dark-75 font-size-lg"> 12 ngày </div>
                            <div class="timeline-badge"><i class="fa fa-genderless text-danger icon-xl"></i></div>
                            <div class="timeline-content font-weight-bolder font-size-lg text-dark-75 pl-3"> ******** : Vừa mua 1 tài khoản Tài khoản TDS 1m Không Cấu Hình (Bảo hành 72h) với giá 13.000đ
                        </div>
                    </div>
                </div>
            </div>
                        <div class="card-body pt-4">
                <div class="timeline timeline-6 mt-3">
                    <div class="timeline-item align-items-start">
                        <div class="timeline-label font-weight-bolder text-dark-75 font-size-lg"> 12 ngày </div>
                            <div class="timeline-badge"><i class="fa fa-genderless text-danger icon-xl"></i></div>
                            <div class="timeline-content font-weight-bolder font-size-lg text-dark-75 pl-3"> ******** : Vừa mua 1 tài khoản Tài khoản TDS 1m Không Cấu Hình (Bảo hành 72h) với giá 0đ
                        </div>
                    </div>
                </div>
            </div>
                        <div class="card-body pt-4">
                <div class="timeline timeline-6 mt-3">
                    <div class="timeline-item align-items-start">
                        <div class="timeline-label font-weight-bolder text-dark-75 font-size-lg"> 13 ngày </div>
                            <div class="timeline-badge"><i class="fa fa-genderless text-danger icon-xl"></i></div>
                            <div class="timeline-content font-weight-bolder font-size-lg text-dark-75 pl-3"> ******** : Vừa mua 1 tài khoản Tài khoản TDS 1m Không Cấu Hình (Bảo hành 72h) với giá 13.000đ
                        </div>
                    </div>
                </div>
            </div>
                        <div class="card-body pt-4">
                <div class="timeline timeline-6 mt-3">
                    <div class="timeline-item align-items-start">
                        <div class="timeline-label font-weight-bolder text-dark-75 font-size-lg"> 14 ngày </div>
                            <div class="timeline-badge"><i class="fa fa-genderless text-danger icon-xl"></i></div>
                            <div class="timeline-content font-weight-bolder font-size-lg text-dark-75 pl-3"> ******** : Vừa mua 2 tài khoản Tài khoản TDS 1m Không Cấu Hình (Bảo hành 72h) với giá 26.000đ
                        </div>
                    </div>
                </div>
            </div>
                    </div>
        </div>
    <div class="col-md-6">
        <div class="card card-custom card-stretch gutter-b">
            <div class="card-header align-items-center border-0 mt-4">
                <h3 class="card-title align-items-start flex-column">
                    <span class="font-weight-bolder text-dark">LỊCH SỬ NẠP TIỀN</span>
                </h3>
            </div>
                        <div class="card-body pt-4">
                <div class="timeline timeline-6 mt-3">
                    <div class="timeline-item align-items-start">
                        <div class="timeline-label font-weight-bolder text-dark-75 font-size-lg"> 11 ngày </div>
                            <div class="timeline-badge"><i class="fa fa-genderless text-danger icon-xl"></i></div>
                            <div class="timeline-content font-weight-bolder font-size-lg text-dark-75 pl-3"> ***enthansanga1 vừa nạp 13000 bởi Crush của Admin 
                        </div>
                    </div>
                </div>
            </div>
                        <div class="card-body pt-4">
                <div class="timeline timeline-6 mt-3">
                    <div class="timeline-item align-items-start">
                        <div class="timeline-label font-weight-bolder text-dark-75 font-size-lg"> 12 ngày </div>
                            <div class="timeline-badge"><i class="fa fa-genderless text-danger icon-xl"></i></div>
                            <div class="timeline-content font-weight-bolder font-size-lg text-dark-75 pl-3"> ***gcon2 vừa nạp 13000 bởi Crush của Admin 
                        </div>
                    </div>
                </div>
            </div>
                        <div class="card-body pt-4">
                <div class="timeline timeline-6 mt-3">
                    <div class="timeline-item align-items-start">
                        <div class="timeline-label font-weight-bolder text-dark-75 font-size-lg"> 13 ngày </div>
                            <div class="timeline-badge"><i class="fa fa-genderless text-danger icon-xl"></i></div>
                            <div class="timeline-content font-weight-bolder font-size-lg text-dark-75 pl-3"> ***h0208 vừa nạp 13000 bởi MOMO (Auto) 
                        </div>
                    </div>
                </div>
            </div>
                        <div class="card-body pt-4">
                <div class="timeline timeline-6 mt-3">
                    <div class="timeline-item align-items-start">
                        <div class="timeline-label font-weight-bolder text-dark-75 font-size-lg"> 14 ngày </div>
                            <div class="timeline-badge"><i class="fa fa-genderless text-danger icon-xl"></i></div>
                            <div class="timeline-content font-weight-bolder font-size-lg text-dark-75 pl-3"> ***uy1998 vừa nạp 50000 bởi Card 
                        </div>
                    </div>
                </div>
            </div>
                        <div class="card-body pt-4">
                <div class="timeline timeline-6 mt-3">
                    <div class="timeline-item align-items-start">
                        <div class="timeline-label font-weight-bolder text-dark-75 font-size-lg"> 14 ngày </div>
                            <div class="timeline-badge"><i class="fa fa-genderless text-danger icon-xl"></i></div>
                            <div class="timeline-content font-weight-bolder font-size-lg text-dark-75 pl-3"> ***cam vừa nạp 20000 bởi Card 
                        </div>
                    </div>
                </div>
            </div>
                        
        </div>
    </div>
</div>
              



        </div>
        </div>
    </div>
</div>
<?php require_once("data/foot.php") ?>